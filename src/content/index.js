    ...
    _inIgnoreList(data) {
        ...
        if (this.rule.ignoredPatterns) {
            for (let i = this.rule.ignoredPatterns.length - 1; i >= 0; i--) {
                ...
                if (data.search(ignoredPattern) !== -1) { return true }
            }
        }
        return false
    }
...
class ExpansionSet {
    ...
    process() {
        this.sources = []
        if (this.sourceNodes && this.sourceNodes.length === this.destinationNodes.length) {
...
class ComicWebsite extends KeyMap {
    ...
    initialize() {
	    ...
	    if (document.readyState === 'complete') {
	    	// load any remainders that were missed above
	        if (!preLoaded) { this.loadUnfinished() }
	        if (!postLoaded) { this.loadFinished() }
	        preLoaded = true
	        postLoaded = true
	    }
    }
    ...
    createNav() {
        ...
        if (prev && !this.prevBound) {
        	...
        	Mousetrap.bind(this.prevCombo, function(e) {
        ...
        if (next && !this.nextBound) {
        	...
        	Mousetrap.bind(this.nextCombo, function(e) {
